    ...
    methods
        function self = get_mpc(self)
            ...
            for n_sc = 1:N_scenarios
                ...
                self.opti.set_value(X{1,n_sc}, x0)
        ...
        function [self, res, feas] = solve_mpc(self, varargin)
            ...
            try
                ...
                var_fns = fieldnames(self.opti_vars);
                for i = 1:length(var_fns)
                    var_name = var_fns{i};
                    var = self.opti_vars.(var_name);
                    [r,c] = size(var);
                    count = 1;
                    if r == 1 && c == 1
                        res.(var_name) = sol.value(var);
                    else
                        var_vals = cell(r*c,1);
                        for j = 1:r
                            for k = 1:c
                                var_vals{count} = sol.value(var{j,k});
                                count = count+1;
                            end
                        end
                        res.(var_name) = var_vals;
                    end
                end

                res.Ufull = res.U; % rename full optimal input trajectory to avoid conflict with existing code
                res.U = res.U{1,1};

                param_fns = fieldnames(self.opti_params);
                for i = 1:length(param_fns)
                    param_name = param_fns{i};
                    param = self.opti_params.(param_name);
                    [r,c] = size(param);
                    count = 1;
                    if r == 1 && c == 1
                        res.(param_name) = sol.value(param);
                    else
                        param_vals = cell(r*c,1);
                        for j = 1:r
                            for k = 1:c
                                param_vals{count} = sol.value(param{j,k});
                                count = count+1;
                            end
                        end
                        res.(param_name) = param_vals;
                    end
                end
                ...
                for i = 1:length(var_fns)
                    ...
                    count = 1;
                    if r == 1 && c == 1
                        res.(var_name) = self.opti.debug.value(var);
                    else
                        var_vals = cell(r*c,1);
                        for j = 1:r
                            for k = 1:c
                                var_vals{count} = self.opti.debug.value(var{j,k});
                                count = count+1;
                            end
                        end
                        res.(var_name) = var_vals;
                ...
                for i = 1:length(param_fns)
                    ...
                    count = 1;
                    if r == 1 && c == 1
                        res.(param_name) = self.opti.debug.value(param);
                    else
                        param_vals = cell(r*c,1);
                        for j = 1:r
                            for k = 1:c
                                param_vals{count} = self.opti.debug.value(param{j,k});
                                count = count+1;
                            end
                        end
                        res.(param_name) = param_vals;
                ...
                % clip input (error in solving ocp, can result in
                % out-of-bound values)
                u = res.U;
        ...
        function self = set_parameters_worst(self, params_list, warn)
            % This method sets the values of the parameters of the
            % optimization problem to those provided as arguments to this
            % method.
