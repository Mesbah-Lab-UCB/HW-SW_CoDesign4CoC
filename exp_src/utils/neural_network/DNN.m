classdef DNN < handle
    ...
    methods
        function [self] = collect_training_data_open_loop(self, c, Nsamp)
            nx = c.prob_info.nx;
            nu = c.prob_info.nu;
            nyc = c.prob_info.nyc;
        ...
        function [self] = collect_training_data_closed_loop(self, c, Nsamp, Nsim)
            nx = c.prob_info.nx;
            ny = c.prob_info.ny;
            nu = c.prob_info.nu;
            nyc = c.prob_info.nyc;
            ...
            x_min = c.prob_info.x_min;
            x_max = c.prob_info.x_max;
            w_min = c.prob_info.w_min;
            w_max = c.prob_info.w_max;
            fp = c.prob_info.fp;
            hp = c.prob_info.hp;
            rand_seed = c.prob_info.rand_seed;
            if strcmp(c.mpc_type, 'economic') || strcmp(c.mpc_type, 'multistage')
            ...
            for i =1:Ncl
                ...
                for k = 1:Nsim
                    ...
                    if strcmp(c.mpc_type, 'economic') || strcmp(c.mpc_type, 'multistage')
                        CEM(:,k+1) = CEM(:,k) + full(CEMadd(Outputs(:,k+1)));
                    end
                end
                % store data
                data_struct = struct;
