...
N = 100;
rand_idx = randi([1,1000],1,N);
disp(N)

% test_in = [43; 50; 0.8];
% test_in_s = scale_inputs(filename, test_in);
test_in_s = loaded_dnn.inputs(:,rand_idx);
...
[r,c] = size(test_out);
for i = 1:c
    [test_out31, test_out32] = plasma_nn_wrapper(test_in_s(1,i),test_in_s(2,i),test_in_s(3,i));
...
disp('Max Abs Error:')
disp(max(max(abs(test_out - test_out3)))/N)
